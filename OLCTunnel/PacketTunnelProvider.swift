...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String: NSObject]?,
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            // 1) DNS теперь идёт ВНУТРИ туннеля (НЕ исключается).
            //    Ядро olcRTC поддерживает SOCKS5 UDP ASSOCIATE и заворачивает DNS
            //    в TCP внутри туннеля (DNS-over-TCP, RFC 7766). Поэтому резолвинг
            //    защищён от операторов, травящих plaintext UDP DNS, и идёт через
            //    VPN, а не в обход него. Прежнее исключение dnsHost /32 убрано.
